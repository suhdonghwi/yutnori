  ...
  return (
    ...
    >
      ...
      <div className="victory-card relative z-[3] isolate w-[min(680px,calc(100vw-40px))] px-7 py-11 text-center text-[#fff3d4] max-[560px]:py-8">
        ...
        <span className="mb-4 block text-[11px] leading-none font-bold tracking-[.24em] text-[color-mix(in_srgb,var(--victory-glow)_62%,#d9c89f)]">{t.victory.tag}</span>
        <strong className="block text-[clamp(52px,7.5vw,88px)] leading-none font-black tracking-[-.045em] text-[#fff0cf] [text-shadow:0_0_30px_color-mix(in_srgb,var(--victory-color)_65%,transparent)]">{t.victory.winner(t.team(winner))}</strong>
        <p className="mt-5 mb-0 text-[clamp(14px,1.3vw,18px)] leading-[1.4] font-semibold text-[#bbae91]">{t.victory.subtitle}</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="throw-result-lockup relative z-[2] isolate w-[min(700px,calc(100vw-24px))] px-8 py-9 text-center max-[560px]:px-4 max-[560px]:py-7">
        <span className="mb-5 block text-[10px] leading-none font-bold tracking-[.25em] text-[color-mix(in_srgb,var(--result-accent)_44%,#bbae91)]">{t.throwEffect.tag}</span>
        <div className="flex items-center justify-center gap-6 max-[560px]:gap-3">
          ...
          <strong className={`block text-[clamp(64px,9vw,112px)] leading-[.9] font-black whitespace-nowrap text-[#fff0cf] [text-shadow:0_0_30px_color-mix(in_srgb,var(--result-accent)_58%,transparent)] ${effect.result.id === "backdo" ? "tracking-[.04em]" : "tracking-[-.055em]"}`}>{t.yut[effect.result.id]}</strong>
