...
function RulesDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="rules-backdrop fixed inset-0 z-80 grid place-items-center bg-night/80 p-6 max-[560px]:p-2.5"
...
export function Lobby({ onStartLocal, onStartAi }: LobbyProps) {
  ...
  return (
    <main className="lobby-shell relative min-h-svh overflow-hidden text-parchment-bright max-[820px]:overflow-auto">
      <div className="absolute top-6 right-8 z-[3] max-[820px]:top-4 max-[820px]:right-4">
        <LanguageSwitcher />
      </div>
      <div
        className="pointer-events-none absolute inset-y-0 right-0 z-0 w-[64vw] [mask-image:linear-gradient(90deg,transparent_0%,black_18%,black_100%)] max-[820px]:top-[76px] max-[820px]:h-[330px] max-[820px]:w-full max-[820px]:[mask-image:linear-gradient(180deg,black_0%,black_72%,transparent_100%)] max-[820px]:opacity-80 max-[560px]:h-[285px]"
        aria-hidden="true"
      >
      ...
      <section className="relative z-[2] mx-auto flex min-h-svh w-[min(1240px,calc(100%-64px))] items-center py-12 max-[820px]:w-[min(620px,calc(100%-36px))] max-[820px]:items-start max-[820px]:pt-10 max-[820px]:pb-16 max-[560px]:w-[calc(100%-28px)]">
        <div className="w-[min(520px,43vw)] max-[820px]:w-full">
          ...
          <div
            className="mt-10 border-y border-gold-soft/20 max-[820px]:mt-[305px] max-[560px]:mt-[260px]"
          ...
          >
            {PLAY_MODES.map((mode) => {
              ...
              return (
                <button
                  key={mode.id}
                  className={`group relative grid min-h-[86px] w-full grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-4 border-b border-gold-soft/10 bg-transparent px-2 text-left transition-[background-color,border-color] duration-200 last:border-b-0 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-gold max-[560px]:min-h-[78px] max-[560px]:grid-cols-[42px_minmax(0,1fr)_auto] max-[560px]:gap-3 ${mode.available ? "cursor-pointer" : "cursor-not-allowed opacity-[.42]"} ${active && mode.available ? "border-b-gold-soft/30 bg-[linear-gradient(90deg,color-mix(in_srgb,var(--color-gold)_12%,transparent),transparent_76%)]" : ""}`}
                  type="button"
                ...
                >
                  ...
                  >
                    <span
                      className={`grid size-11 place-items-center rounded-full border transition-[color,border-color,background-color,box-shadow] duration-200 max-[560px]:size-10 ${active && mode.available ? "border-gold bg-gold/10 text-parchment shadow-[0_0_24px] shadow-gold/10" : "border-gold-soft/20 text-parchment-faint"}`}
                    >
