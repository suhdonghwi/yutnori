    ...
    >
      ...
      >
        ...
        <div className="min-w-0">
          <small className="mb-1 block text-label font-medium text-parchment-dim max-sm:mb-0 max-sm:text-micro">
            {t.dock.now}
          </small>
          <strong className="block overflow-hidden text-[clamp(16px,1.5vw,21px)] leading-[1.18] font-extrabold text-ellipsis whitespace-nowrap text-parchment-bright max-sm:max-w-[calc(100vw-62px)] max-sm:text-body">
            {statusText}
          </strong>
        </div>
      </DockSection>
      ...
      {phase === "move" && isAiTurn ? (
        <DockSection
          ...
          ariaLive="polite"
        >
          <span className="grid size-10 shrink-0 place-items-center rounded-full border border-coral/35 text-label font-black text-coral max-sm:size-8">
            AI
          </span>
          <div>
            ...
            <strong className="block text-sm leading-[1.2] font-extrabold whitespace-nowrap text-parchment-bright max-sm:max-w-[68vw] max-sm:overflow-hidden max-sm:text-label max-sm:text-ellipsis">
            ...
            return (
              ...
              >
                {group.length > 1 && !follower
                  ...
                  : t.dock.token(index)}
                <span className="mt-1.5 block text-micro font-medium text-parchment-faint group-hover:text-parchment-dim max-sm:mt-0.5">
                  {follower
        ...
        >
          <button
            type="button"
            className="cursor-pointer border-0 border-r border-gold/30 bg-gold/10 px-5 py-3 text-sm font-extrabold text-gold transition-colors hover:bg-gold/15 max-sm:min-w-0 max-sm:px-1.5 max-sm:py-1.75 max-sm:text-label"
          ...
          <button
            type="button"
            className="cursor-pointer border-0 bg-transparent px-5 py-3 text-sm font-extrabold text-parchment-dim transition-colors hover:bg-white/[.04] hover:text-parchment-bright max-sm:min-w-0 max-sm:px-1.5 max-sm:py-1.75 max-sm:text-label"
        ...
        <DockSection
          ratio={1.2}
          minWidth="240px"
          className="max-sm:min-h-[56px]"
        >
          {phase === "gameover" ? (
            <button
              className="flex size-full cursor-pointer items-center justify-center gap-3 border-0 bg-transparent px-6 font-black text-gold transition-colors hover:bg-gold/10 hover:text-parchment-bright max-sm:text-sm"
              type="button"
              onClick={onReset}
            >
              {t.dock.playAgain}{" "}
            ...
            <button
              className="flex size-full cursor-pointer items-center justify-center gap-3 border-0 bg-transparent px-6 font-black text-gold transition-colors enabled:hover:bg-gold/10 enabled:hover:text-parchment-bright disabled:cursor-wait disabled:opacity-45 max-sm:px-3 max-sm:text-sm"
              type="button"
              onClick={onThrow}
